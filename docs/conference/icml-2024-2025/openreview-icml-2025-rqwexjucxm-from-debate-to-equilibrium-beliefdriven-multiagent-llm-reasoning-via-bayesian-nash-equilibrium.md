...
title_zh: 从辩论到均衡：基于贝叶斯纳什均衡的信念驱动多智能体大模型推理
...
score: 8.0
evidence: 基于贝叶斯纳什均衡的多智能体协调路由
tldr: 针对多智能体推理计算成本高且缺乏收敛保证的问题，ECON将多LLM协调建模为不完全信息博弈，求解贝叶斯纳什均衡。该方法采用分层强化学习范式，每个智能体基于对他人策略的信念独立选择最优响应，最终集中输出。实验表明ECON在保证收敛性的同时显著降低了计算开销，并在多个推理任务上验证了其有效性。
...
motivation: 多智能体推理计算成本高且缺乏收敛保证。
method: 将协调建模为不完全信息博弈，求解贝叶斯纳什均衡。
result: 在多个推理任务上实现更高效且可收敛的协调。
conclusion: 博弈论视角为多智能体协调提供了理论基础和高效方案。
---
