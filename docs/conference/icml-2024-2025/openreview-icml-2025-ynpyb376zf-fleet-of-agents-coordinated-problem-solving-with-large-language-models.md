...
score: 8.0
evidence: 通过粒子滤波选择实现动态智能体路由
tldr: 现有LLM多智能体框架在成本与质量间难以平衡。本文提出Fleet of Agents (FoA)，利用遗传式粒子滤波方法，多个智能体动态探索搜索树，通过启发式价值函数重采样选择最优路径。实验表明FoA能在控制成本的同时显著提升推理质量。该方法为多智能体系统中的高效任务路由提供了新范式。
...
motivation: 现有LLM多智能体框架难以平衡成本与质量，缺乏动态路由机制。
method: 提出Fleet of Agents框架，使用遗传式粒子滤波在动态树搜索中调度多个智能体探索路径，并通过重采样优化。
result: 在多个推理任务上，FoA在保持较低计算成本的同时取得了优于基线方法的准确率。
conclusion: FoA通过动态路由有效平衡了探索与利用，为多智能体系统提供了一种高效的协调机制。
---
