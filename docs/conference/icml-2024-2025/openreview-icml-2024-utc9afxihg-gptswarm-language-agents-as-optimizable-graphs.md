...
score: 8.0
evidence: 优化边连接以实现动态智能体路由
tldr: 该论文将LLM智能体形式化为可优化的计算图，其中边表示信息流与智能体间的协作。通过自动图优化器（节点优化和边优化），GPTSwarm能够动态调整智能体间的连接方式，实现高效的任务路由和编排。实验表明，该方法在多个任务上优于静态设计的智能体系统，为多智能体路由提供了灵活的解决方案。
...
motivation: 大量人工设计的提示工程方法缺乏统一视角，尤其是智能体间的动态路由难以手动优化。
method: 将LLM智能体建模为计算图，通过自动边优化调整智能体间的连接和路由。
result: 在多个基准上，GPTSwarm提升了任务性能，展示了动态路由的有效性。
conclusion: 图优化可自动改进多智能体系统中的任务分配和协作路由。
---
